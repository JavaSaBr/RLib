...
public class WindowDragHandler {
  ...
  protected void processMove(MouseEvent event) {
    ...
    if (dragOffset == null) {
      LOGGER.debug("processMove -> dragOffset -> null");
      return;
    }

    LOGGER.debug(dragOffset, offset -> "processMove -> dragOffset -> " + offset);
    ...
    LOGGER.debug(
        dragOffset,
        event,
        (offset, ev) -> "processMove -> dragXY -> " + (ev.getScreenX() - offset.getX()) + "-" + (ev.getScreenY()
                                                                                                     - offset.getY()));
  ...
  protected void processStartDrag(MouseEvent event) {
    ...
    LOGGER.debug(this, handler -> "processStartDrag -> initXY -> " + handler.getStartX() + ", " + handler.getStartY());

    var dragOffset = new Point2D(event.getScreenX(), event.getScreenY());

    LOGGER.debug(dragOffset, offset -> "processStartDrag -> dragOffset -> " + offset);
