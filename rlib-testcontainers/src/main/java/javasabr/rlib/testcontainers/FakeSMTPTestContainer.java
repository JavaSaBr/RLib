...
public class FakeSMTPTestContainer extends GenericContainer<FakeSMTPTestContainer> {
  ...
  public long getEmailCountFrom(@NotNull String email) {
    ...
    try {
      ...
      var response = httpClient.send(request, responseInfo -> BodySubscribers.ofString(StandardCharsets.UTF_8));
  ...
  public boolean deleteEmails() {
    ...
    var request = HttpRequest
        .newBuilder(URI.create(getBaseUrl() + "/emails"))
        .DELETE()
        .build();

    try {

      var response = httpClient.send(request, responseInfo -> BodySubscribers.discarding());
