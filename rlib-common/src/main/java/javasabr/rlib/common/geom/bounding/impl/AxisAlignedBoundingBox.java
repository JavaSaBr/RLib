...
import javasabr.rlib.common.geom.Matrix3f;
...
public class AxisAlignedBoundingBox extends AbstractBounding {
  ...
  public boolean contains(float x, float y, float z) {
    return Math.abs(getResultCenterX() - x) < sizeX && Math.abs(getResultCenterY() - y) < sizeY
   ...
   * @return AABB's size.
   */
  public Vector3f getSize(Vector3fBuffer buffer) {
    return buffer
        .nextVector()
        .set(size);
  }
