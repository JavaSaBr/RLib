...
import javasabr.rlib.common.geom.util.AngleUtils;
...
public class Quaternion4f {
   ...
   * axes. These axes are assumed to be orthogonal and no error checking is applied. Thus, the user must insure that the
   * three axes being provided indeed represents a proper right handed coordinate system.
   *
  ...
  public final Vector3f multLocal(Vector3f vector) {
    ...
    vector.setX(w * w * vectorX + 2 * y * w * vectorZ - 2 * z * w * vectorY + x * x * vectorX + 2 * y * x * vectorY
        + 2 * z * x * vectorZ - z * z * vectorX - y * y * vectorX);
    vector.setY(2 * x * y * vectorX + y * y * vectorY + 2 * z * y * vectorZ + 2 * w * z * vectorX - z * z * vectorY
        + w * w * vectorY - 2 * x * w * vectorZ - x * x * vectorY);
    vector.setZ(2 * x * z * vectorX + 2 * y * z * vectorY + z * z * vectorZ - 2 * w * y * vectorX - y * y * vectorZ
