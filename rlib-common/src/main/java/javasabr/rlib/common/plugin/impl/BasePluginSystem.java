...
import javasabr.rlib.common.plugin.exception.InitializePluginException;
import javasabr.rlib.common.plugin.exception.PluginException;
...
public class BasePluginSystem implements ConfigurablePluginSystem {
  ...
  @Override
  public CompletableFuture<ConfigurablePluginSystem> preLoad() {
    return preLoad(ForkJoinPool.commonPool());
  }

  @Override
  public CompletableFuture<ConfigurablePluginSystem> preLoad(Executor executor) {
  ...
  @Override
  public CompletableFuture<ConfigurablePluginSystem> initialize() {
    return initialize(ForkJoinPool.commonPool());
  }

  @Override
  public CompletableFuture<ConfigurablePluginSystem> initialize(Executor executor) {
   ...
   */
  private CompletableFuture<Plugin> createPluginClass(PluginContainer container, Executor executor) {
   ...
   */
  protected CompletableFuture<Array<PluginContainer>> loadPlugins(Path path, Executor executor, boolean embedded) {
   ...
   */
  protected CompletableFuture<@Nullable PluginContainer> loadPlugin(
      Path directory,
    ...
    return supplyAsync(
        () -> {
          ...
          if (pluginImplementations.isEmpty()) {
            LOGGER.warning(
                directory,
                arg -> "Can't load the plugin from the directory" + arg
          ...
          if (appVersion != null) {
            ...
            if (minVersion.compareTo(appVersion) > 0) {
              LOGGER.warning(
                  description,
                  arg -> "Can't load the plugin " + arg.id() + ", it requires the app version " + arg.minAppVersion()
                      + " or higher.");
              return null;
            }
          }
   ...
   */
  protected Optional<Path> getEmbeddedPluginsPath() {
   ...
   */
  protected Optional<Path> getInstallationPluginsPath() {
  ...
  public Array<PluginContainer> getPluginContainers() {
    return state.get().idToContainer.values(PluginContainer.class);
  }

  @Override
  public @Nullable PluginContainer getPluginContainer(String id) {
  ...
  @Override
  public @Nullable Plugin getPlugin(String id) {
    return state.get().idToPlugin.get(id);
  }

  @Override
  public @Nullable Plugin installPlugin(Path file, boolean needInitialize) {
    ...
    var container = loadPlugin(pluginFolder, baseLoader, ForkJoinPool.commonPool(), false).join();
