 ...
 * @author Ronn
 */
public class ConcurrentLongDictionary<V> extends AbstractLongDictionary<V> implements ConcurrentDictionary<LongKey, V> {
	...
	protected ConcurrentLongDictionary() {
		this(Dictionary.DEFAULT_LOAD_FACTOR, Dictionary.DEFAULT_INITIAL_CAPACITY);
	}

	protected ConcurrentLongDictionary(final float loadFactor) {
		this(loadFactor, Dictionary.DEFAULT_INITIAL_CAPACITY);
	}

	protected ConcurrentLongDictionary(final float loadFactor, final int initCapacity) {
		super(loadFactor, initCapacity);
		this.size = new AtomicInteger();
		this.locker = createLocker();
	}

	protected ConcurrentLongDictionary(final int initCapacity) {
