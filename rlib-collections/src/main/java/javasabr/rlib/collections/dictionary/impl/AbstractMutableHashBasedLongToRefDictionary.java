...
import javasabr.rlib.collections.dictionary.MutableLongToRefDictionary;
import javasabr.rlib.collections.dictionary.UnsafeMutableLongToRefDictionary;
...
public abstract class AbstractMutableHashBasedLongToRefDictionary<V, E extends LinkedHashLongToRefEntry<V, E>>
    extends AbstractHashBasedLongToRefDictionary<V, E> implements UnsafeMutableLongToRefDictionary<V, E> {
  ...
  @Nullable
  @Override
  public <T> V getOrCompute(long key, T arg1, Function<T, V> factory) {
    E entry = findEntry(key);
    if (entry != null) {
      return entry.value();
    }
  ...
  @Override
  public void putAll(LongToRefDictionary<? extends V> dictionary) {
    if (dictionary instanceof AbstractHashBasedLongToRefDictionary<?, ?> hashBased) {
  ...
  @Override
  public MutableLongToRefDictionary<V> append(LongToRefDictionary<? extends V> dictionary) {
    putAll(dictionary);
    return this;
  }
  ...
  @Nullable
  protected E removeEntryForKey(long key) {
  ...
  protected void addEntry(int hash, long key, @Nullable V value, int index) {
  ...
  protected abstract E allocate(int hash, long key, @Nullable V value, @Nullable E next);
